public static class Dominoes
{
    public static bool CanChain(IEnumerable<(int, int)> dominoes)
    {
        ...
        return Solve(new List<(int,int)>(rest), normal) || Solve(new List<(int, int)>(rest), flipped);
    }

    private static bool Solve(List<(int,int)> remaining, List<(int,int)> chain)
    {
        ...
        var currentRightEnd = chain[chain.Count - 1].Item2;
        ...
        for(int i = 0; i < remaining.Count; i++)
        {
            ...
            {
                if(oriented.Item1 == currentRightEnd)
                {
